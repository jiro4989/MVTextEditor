...
class IconSetManager {
  ...
  IconSetManager(MainController mc, GridPane igp, ImageView iiv, Label ifl, Label isl) { // {{{
    ...
    iconImageView.setOnMouseMoved(
        e -> {
          iconImageViewOnMouseMoved(e);
        });
    iconFocusLabel.setOnMouseClicked(
        e -> {
          iconFocusLabelOnMouseClicked(e);
        });
    iconSelectedLabel.setOnMouseClicked(
        e -> {
          iconSelectedLabelOnMouseClicked(e);
        });

    iconGridPane.setOnKeyPressed(
        e -> {
          if (KeyCode.H == e.getCode() && !e.isControlDown() && !e.isShiftDown()) {
            moveFocusGridPane(0, -1);
          } else if (KeyCode.J == e.getCode() && !e.isControlDown() && !e.isShiftDown()) {
            moveFocusGridPane(1, 0);
          } else if (KeyCode.K == e.getCode() && !e.isControlDown() && !e.isShiftDown()) {
            moveFocusGridPane(-1, 0);
          } else if (KeyCode.L == e.getCode() && !e.isControlDown() && !e.isShiftDown()) {
            moveFocusGridPane(0, 1);
  ...
  private void iconImageViewOnMouseMoved(MouseEvent e) { // {{{
    iconWidthOpt.ifPresent(
        iconWidth -> {
          iconHeightOpt.ifPresent(
              iconHeight -> {
                int x = (int) e.getX();
                int y = (int) e.getY();
                ...
                if (gridX <= iconWidth && gridY <= iconHeight) {
